...
public class PassPredictor {
	 ...
	 */
	public PassPredictor(final TLE theTLE, final GroundStationPosition theQTH) throws IllegalArgumentException,
			InvalidTleException, SatNotFoundException {
	...
	public SatPassTime nextSatPass(final Date date, final boolean windBack) throws InvalidTleException, SatNotFoundException {
		...
		return new SatPassTime(startDate, endDate, tca, polePassed, aosAzimuth, losAzimuth,
				(maxElevation / (2.0 * Math.PI)) * 360.0);
	 ...
	 * @throws SatNotFoundException
	 */
	private SatPos getPosition(final Calendar cal, final int offSet) throws InvalidTleException, SatNotFoundException {
		SatPos satPos;
	...
	public List<SatPos> getPositions(final Date referenceDate, final int incrementSeconds, final int minutesBefore,
			final int minutesAfter) throws InvalidTleException, SatNotFoundException {
